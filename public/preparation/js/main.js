    ...
    function init() {
        ...
        $('#all-pages').dlmenu({
            onLinkClick: function (el, ev) {
                ...
                if (
                    openPage !== $pages.eq(current).index() &&
                    openPage !== -1
                ) {
        ...
        $('nav .navbar-brand').on('click', function (el) {
            el.preventDefault();
        ...
        $('.prev-page').on('click', function (el) {
            el.preventDefault();
            prevPage(21);
        });
...
CanvasClip.prototype.clipCanvas = function () {
    ...
    if (!self._getRandom()) {
        x1End = getRandomInt(
            (30 * self._getWidth()) / 100,
            self._getWidth() - (20 * self._getWidth()) / 100,
        );
        ...
        y2End = getRandomInt(
            (30 * self._getHeight()) / 100,
            self._getHeight() - (20 * self._getHeight()) / 100,
        );
    ...
    if (self._getRandom()) {
        ...
        x2End = getRandomInt(
            self._getWidth() / 1.5,
            self._getWidth() - (30 * self._getWidth()) / 100,
        );
        x2Start = self._getPoint(1, 0);
        ...
        y3End = getRandomInt(
            self._getHeight() / 1.5,
            self._getHeight() - (30 * self._getHeight()) / 100,
        );
        y3Start = self._getPoint(2, 1);
    ...
    if (canvas.getContext) {
        ...
        this._animate({
            ...
            timing: function (timeFraction) {
                return (
                    Math.pow(timeFraction, 2) * Math.pow(timeFraction - 2, 2)
                );
            },
...
CanvasClip.prototype.openFullCanvas = function (position) {
    ...
    if ($('body').hasClass('inverse')) windowHeight = window.innerHeight - 80;

    var canvas = document.querySelector('.full-canvas');

    if (!canvas) {
        $('body').append(
            '<canvas class="full-canvas" width=' +
                windowWidth +
                ' height=' +
                windowHeight +
    ...
    try {
        if (canvas.getContext) {
            ...
            if (self._getFit() === 'contain') {
                ...
                if (left <= minLeft) leftWidth = minLeft - left;

                if (left > minLeft) leftWidth = left - minLeft;

                if (top <= minTop) topWidth = minTop - top;

                if (top > minTop) topWidth = top - minTop;

                if (right <= minRight) rightWidth = minRight - right;

                if (bottom <= minBottom) bottomWidth = minBottom - bottom;

                if (bottom > minBottom) bottomWidth = bottom - minBottom;

                if (right > minRight) rightWidth = right - minRight;
            }
            ...
            self._animateFull({
                ...
                draw: function (progress) {
                    ...
                    x2pos =
                        x2pos < windowWidth
                            ? windowWidth - x2interval + progress * x2interval
                            : windowWidth;
                    y2pos = y2pos > 0 ? y2interval - progress * y2interval : 0;

                    x3pos =
                        x3pos < windowWidth
                            ? windowWidth - x3interval + progress * x3interval
                            : windowWidth;
                    y3pos =
                        y3pos < windowHeight
                            ? windowHeight - y3interval + progress * y3interval
                            : windowHeight;

                    x4pos = x4pos > 0 ? x4interval - progress * x4interval : 0;
                    y4pos =
                        y4pos < windowHeight
                            ? windowHeight - y4interval + progress * y4interval
                            : windowHeight;
                    ...
                    if (self._getFit() === 'cover') {
                        top =
                            top > minTop
                                ? topWidth + minTop - progress * topWidth
                                : minTop;
                        left =
                            left > minLeft
                                ? leftWidth + minLeft - progress * leftWidth
                                : minLeft;
                        bottom =
                            bottom < minBottom
                                ? imgHeight +
                    ...
                    if (self._getFit() === 'contain') {
                        ...
                        right =
                            right > minRight
                                ? rightWidth + minRight - progress * rightWidth
        ...
        if (canvas.getContext) {
            ...
            self._animate({
                ...
                draw: function (progress) {
                    ...
                    x2Start =
                        x2Start > x2End
                            ? canvasWidth - x2Width * progress
                            : x2End;
                    y2Start = y2Start < y2End ? y2End * progress : y2End;

                    x3Start =
                        x3Start > x3End
                            ? canvasWidth - x3Width * progress
                            : x3End;
                    y3Start =
                        y3Start > y3End
                            ? canvasHeight - y3Width * progress
                            : y3End;

                    x4Start = x4Start < x4End ? x4End * progress : x4End;
                    y4Start =
                        y4Start > y4End
                            ? canvasHeight - y4Width * progress
                            : y4End;
...
CanvasClip.prototype._animateFull = function (options) {
    ...
    window.requestAnimationFrame(function animate() {
        ...
        var progress = options.timing(timeFraction);
        options.draw(progress);
        if (timeFraction < 1) window.requestAnimationFrame(animate);
    });
};
...
CanvasClip.prototype._createElement = function (block, img) {
    ...
    block.append(
        '<canvas width=' +
            ...
            ' height=' +
            self._getHeight() +
            ' class="canvaClip">update your browser</canvas>',
    );
    ...
    if (canvas.getContext) {
        ...
        if (!imgWidth) imgWidth = $(self._getImage()).innerWidth();

        if (!imgHeight) imgHeight = $(self._getImage()).innerHeight();
...
CanvasClip.prototype._setPoints = function () {
    ...
    self._points = [
        [0, 0],
        [self._getWidth(), 0],
        [self._getWidth(), self._getHeight()],
        [0, self._getHeight()],
    ];
};
...
CanvasClip.prototype.onMouseEnter = function (callback) {
    ...
    if (!navigator.userAgent.match(/(iPhone|iPod|iPad|Android|BlackBerry)/)) {
        $(self._getElement()).on('mouseenter', function () {
            self.clipCanvas();
            if (callback) {
                callback();
            }
...
CanvasClip.prototype.onMouseLeave = function (callback) {
    ...
    if (!navigator.userAgent.match(/(iPhone|iPod|iPad|Android|BlackBerry)/)) {
        $(self._getElement()).on('mouseleave', function () {
            self.unclipCanvas();
            if (callback) {
                callback();
            }
...
CanvasClip.prototype.onMouseMove = function () {
    $(this._getElement()).on('mousemove', function () {
        console.log('mousemove');
    });
};
CanvasClip.prototype.onClick = function (
    canvasObject,
    block,
    count,
    index,
    ...
    $(self._getElement()).on('mousedown', function (e) {
        left = e.clientX;
        top = e.clientY;
    });
    ...
    $(self._getElement())
        ...
        .on('mouseup', function (e) {
            ...
            if (
                Math.ceil(left) !== Math.ceil(left2) ||
                Math.ceil(top) !== Math.ceil(top2)
            )
                return;
            ...
            $(block)
                .owlCarousel()
                .data('owlCarousel').settings.autoplay = false; // stop autoplay carousel
            ...
            $(block)
                ...
                .on('resized.owl.carousel', function () {
                    ...
                    if (startActiveIndex >= plus) startActiveIndex = 0;

                    if (startActiveIndex < 0) startActiveIndex = plus;
                    ...
                    canvasObject[count][numeric].resetFullCanvas(
                        block,
                        position,
                    );
                });
            ...
            $(block)
                ...
                .on('translated.owl.carousel', function () {
                    ...
                    if (
                        $(block).owlCarousel().data('owlCarousel').settings
                            .loop &&
                        plus <= $(block).find('.owl-item.active').length
                    )
                        numeric = endActiveIndex + plus;

                    if (
                        !$(canvasObject[count][numeric]._getElement())
                            .parent()
                            .parent()
                            .parent()
                            .hasClass('active')
                    ...
                    $(block).trigger('to.owl.carousel', [
                        endActiveIndex - startActiveIndex,
                        1,
                        true,
                    ]);
            ...
            setTimeout(function () {
                ...
                var fotorama = $(block)
                    .parent()
                    .find('.fotorama')
                    .data('fotorama'); // fotorama's object
                ...
                if ($fotoramaBottom.find('.bottom').length <= 0)
                    // add close button
                    $fotoramaBottom.append(
                        '<div class="bottom"><a href="#" class="close-this"></a></div>',
                    );
                ...
                setTimeout(function () {
                    // show fullscreen fotorama after some time
                ...
                $fototramaShowIcons.off('click').on('click', function (e) {
                    e.preventDefault();
                    $(this).addClass('open');
                    $(this).parent().find('ul').addClass('open');
                });
                ...
                function closeFullFotorama(e) {
                    ...
                    if (isShowEndFotorama) {
                        if (
                            ...
                            position = $(block)
                                ...
                                .find('.canvaClip')
                                .offset();
                            canvasObject[count][endActiveIndex].resetFullCanvas(
                                block,
                                position,
                            );
                    ...
                    setTimeout(function () {
                        ...
                        if ($(window).width() > 767) {
                            ...
                            }, canvasObject[count][index]._getFullDuration() +
                                50);
                        } else {
                            if (
                                !$(block)
                        ...
                        //setTimeout(function(){
                        canvasObject[count][numeric].closeFullCanvas(
                            position,
                            block,
                        );
                        //}, 100);
...
function initClipCanvas(block, count, option) {
    ...
    if ($(window).width() > 767) {
        ...
        $items.each(function (index, item) {
            ...
            addEvent(img, 'load', function () {
                ...
                canvasObject[count][index]._setFullDuration(
                    option.durationFull,
                );

                canvasObject[count][index]._setFit(option.fit);

                canvasObject[count][index].onMouseEnter(function () {
                    $(canvasObject[count][index]._getElement())
                        .parent()
                        .parent()
                        .addClass('hover');
                });

                canvasObject[count][index].onMouseLeave(function () {
                    $(canvasObject[count][index]._getElement())
                        .parent()
                        .parent()
                        .removeClass('hover');
                ...
                canvasObject[count][index].onClick(
                    canvasObject,
                    block,
                    count,
                    index,
                    canvasObject[count][index]._getFit(),
                );
    ...
    function resize() {
        ...
        globalTimer = setTimeout(function () {
            ...
            owl.each(function (index, carousel) {
...
function initFotorama(owl, count) {
    ...
    for (i; i < itemsImages.length; i++) {
        ...
        if (
            itemsImages[i].parentNode.parentNode.parentNode.className.indexOf(
                'clone',
            ) === -1
        ) {
            fotoramaImages[j] = itemsImages[i]; // записується у масив
            var p =
                itemsImages[i].parentNode.parentNode.getElementsByClassName(
                    'item-info',
                )[0];
    ...
    if (
        !$(owl).parent().find('.fotorama').length &&
        !$(owl).parent().find('.fotorama--hidden').length
    ) {
    ...
    for (var i = 0; i < fotoramaImages.length; i++) {
        ...
        if (fotorama_div[i]) {
            ...
            if (
                !$(fotorama_div[i]).find('.item-info').html() &&
                !$(fotorama_div[i]).find('.item-info').length
            ) {
            ...
            $(owl)
                ...
                .find('.fotorama')
                .append(
                    '<div class="fotorama-info" data-thumb="' +
                        src +
                        '" data-img="' +
                        src +
                        '"> ' +
    ...
    function runFotorama() {
        ...
        if ($(window).width() <= 767) {
            if (!$owl.hasClass('hiden')) $owl.addClass('hiden');

            if ($('.full-canvas').length) $('.full-canvas').remove();
            ...
            if (fotorama !== undefined) {
                ...
                $showIcons.off().on('click', function (e) {
                    e.preventDefault();
                    $(this).addClass('open');
                    $(this).parent().find('ul').addClass('open');
                    $(this)
                        ...
                        if ($(item).hasClass('fotorama__active')) {
                            if (
                                !$(item)
                                    .parent()
                                    .parent()
                                    .find('.fotorama_mobile-count').length
                            ) {
                                $(item)
                                    .parent()
                                    .parent()
                                    .append(
                                        '<p class="fotorama_mobile-count">' +
                                            index +
                                            1 +
                                            ' / ' +
                                            $(item)
        ...
        function mobileFotorama() {
            ...
            $active.off('touchstart').on('touchstart', function (e) {
            ...
            $active.off('touchend').on('touchend', function (e) {
                var touchobj = e.originalEvent; // reference first touch point (ie: first finger)
                ...
                $fotoramaNav
                    ...
                    .on('click', function (e) {
                        ...
                        var $fotoramaShowIcons = $fotoramaMobile.find(
                            '.fotorama_show-icons',
                        );
...
function initCarousel(owl) {
    $(owl).each(function (index, carousel) {
        ...
        function resize() {
            if (!$(carousel).hasClass('demo')) setBlockHeight(carousel);
            recalcCarouselWidth(carousel);
        }
...
function runClipCanvas() {
    ...
    if (owl.hasClass('clipRunned')) return;
    ...
    if (wWidth > 991) {
        var itemImg = document.querySelectorAll(
            '.pt-page-current .clipCarousel .item_img',
        );
        if (itemImg) {
            ...
            if (
                $body.hasClass('inverse') ||
                $body.hasClass('color_bg') ||
                $body.hasClass('gradient_bg')
            ) {
    ...
    owl.each(function (index, carousel) {
        ...
        initClipCanvas($(carousel), number + index, {
...
DaysLeft.prototype = {
                ...
                draw: function (progress) {
                    ...
                    if (self._getData()) {
                        ...
                        timer.renderText(
                            daysLeft.toString(),
                            self._getWidth() / 2 -
                                timer.measureText(daysLeft).width / 2 -
                                5,
                            self._getHeight() / 2,
                            5,
                        );
                    ...
                    timer.renderText(
                        self._getText(),
                        self._getWidth() / 2 -
                            timer.measureText(self._getText()).width / 2 -
                            15,
                        95,
                        4,
                    );
                ...
                draw: function (progress) {
                    ...
                    if (self._getPoint(0, 0) >= self.elipseX1)
                        self._setPoint(
                            ...
                            self.elipseX1 +
                                self.elipseX1Width -
                                (self.elipseX1Width * progress) / 100,
                        );
                    if (self._getPoint(0, 0) < self.elipseX1)
                        self._setPoint(
                            0,
                    ...
                    if (self._getPoint(0, 1) >= self.elipseY1)
                        self._setPoint(
                            ...
                            self.elipseY1 +
                                self.elipseY1Width -
                                (self.elipseY1Width * progress) / 100,
                        );
                    if (self._getPoint(0, 1) < self.elipseY1)
                        self._setPoint(
                            0,
                    ...
                    if (self._getPoint(1, 0) >= self.elipseX2)
                        self._setPoint(
                            ...
                            self.elipseX2 +
                                self.elipseX2Width -
                                (self.elipseX2Width * progress) / 100,
                        );
                    if (self._getPoint(1, 0) < self.elipseX2)
                        self._setPoint(
                            1,
                    ...
                    if (self._getPoint(1, 1) >= self.elipseY2)
                        self._setPoint(
                            ...
                            self.elipseY2 +
                                self.elipseY2Width -
                                (self.elipseY2Width * progress) / 100,
                        );
                    if (self._getPoint(1, 1) < self.elipseY2)
                        self._setPoint(
                            1,
                    ...
                    if (self._getPoint(2, 0) >= self.elipseX3)
                        self._setPoint(
                            ...
                            self.elipseX3 +
                                self.elipseX3Width -
                                (self.elipseX3Width * progress) / 100,
                        );
                    if (self._getPoint(2, 0) < self.elipseX3)
                        self._setPoint(
                            2,
                    ...
                    if (self._getPoint(2, 1) >= self.elipseY3)
                        self._setPoint(
                            ...
                            self.elipseY3 +
                                self.elipseY3Width -
                                (self.elipseY3Width * progress) / 100,
                        );
                    if (self._getPoint(2, 1) < self.elipseY3)
                        self._setPoint(
                            2,
                    ...
                    if (self._getPoint(3, 0) >= self.elipseX4)
                        self._setPoint(
                            ...
                            self.elipseX4 +
                                self.elipseX4Width -
                                (self.elipseX4Width * progress) / 100,
                        );
                    if (self._getPoint(3, 0) < self.elipseX4)
                        self._setPoint(
                            3,
                    ...
                    if (self._getPoint(3, 1) >= self.elipseY4)
                        self._setPoint(
                            ...
                            self.elipseY4 +
                                self.elipseY4Width -
                                (self.elipseY4Width * progress) / 100,
                        );
                    if (self._getPoint(3, 1) < self.elipseY4)
                        self._setPoint(
                            3,
    ...
    _setPath: function () {
        ...
        if (self._getRandom()) {
            ...
            self.y2Rand = getRandomInt(
                self._getHeight() / n,
                self._getHeight() - self._getHeight() / n,
            );
            self.y2Width = Math.abs(self._getPoint(1, 1) - self.y2Rand);
            self.x3Rand = getRandomInt(
                self._getWidth() / n,
                self._getWidth() - self._getWidth() / n,
            );
        ...
        if (!self._getRandom()) {
            self.x1Rand = 5;
            self.x1Width = Math.abs(self._getPoint(0, 0) - self.x1Rand);
            self.y1Rand = getRandomInt(
                self._getHeight() / n,
                self._getHeight() - self._getHeight() / n,
            );
            self.y1Width = Math.abs(self._getPoint(0, 1) - self.y1Rand);
            self.x2Rand = getRandomInt(
                self._getWidth() / n,
                self._getWidth() - self._getWidth() / n,
            );
            self.x2Width = Math.abs(self._getPoint(1, 0) - self.x2Rand);
            ...
            self.x3Width = Math.abs(self._getPoint(2, 0) - self.x3Rand);
            self.y3Rand = getRandomInt(
                self._getHeight() / n,
                self._getHeight() - self._getHeight() / n,
            );
            self.y3Width = Math.abs(self._getPoint(2, 1) - self.y3Rand);
            self.x4Rand = getRandomInt(
                self._getWidth() / n,
                self._getWidth() - self._getWidth() / n,
            );
            self.x4Width = Math.abs(self._getPoint(3, 0) - self.x4Rand);
    ...
    _setPathElipse: function () {
        ...
        self.elipseX1 =
            self._getWidth() / 2 +
            (self._getWidth() / 2) * Math.cos(elipseAngle);
        self.elipseX1Width = Math.abs(self._getPoint(0, 0) - self.elipseX1);
        self.elipseY1 =
            self._getHeight() / 2 +
            (self._getHeight() / 2) * Math.sin(elipseAngle);
    ...
    _setNumberColor: function (color) {
        if (color) this.numberColor = color;
    },
    _getNumberColor: function () {
        return this.numberColor;
    },
    _setNumberFont: function (size) {
        if (size) this.numberFont = size;
    },
...
(function () {
    $('.subscribe-btn').on('click', function (e) {
        ...
        document.forms['subscribe-form']['subscribe'].placeholder =
            'ENTER YOUR EMAIL';
    ...
    var showSocialIcons = function () {
        $('.navbar_show-icons')
            ...
            .on('click', function (e) {
                ...
                if (
                    this.parentNode.getElementsByClassName('navbar_icons')
                        .length
                ) {
                    var ul = this.parentNode
                        .getElementsByClassName('navbar_icons')[0]
            ...
            $('.navbar-toggle')
                ...
                .on('click', function (e) {
                    ...
                    if (!$('nav').hasClass('open')) {
                        ...
                        if (
                            $('ul.nav').find('li').height() *
                                $('ul.nav').find('li').length >
                            $(window).height()
                        ) {
...
function hideAll() {
    ...
    if (main) main.className += ' invis';

    if (nav) nav.className += ' invis';
}
function showAll() {
    var main = document.querySelector('#pt-main'),
        nav = document.querySelector('.navbar');

    if (main) main.className = main.className.replace(' invis', '');

    if (nav) nav.className = nav.className.replace(' invis', '');
}
...
$(document).ready(function () {
    ...
    if (document.forms['subscribe-form']) {
        addEvent(
            document.forms['subscribe-form']['subscribe'],
            'click',
            function (event) {
        ...
        addEvent(
            document.forms['subscribe-form']['subsubmit'],
            'click',
            function (event) {
                event = event || window.event;
                event.stopPropagation();
    ...
    $('.notify-me').submit(function (e) {
        ...
        if (value.length <= 0 || !validateEmail(value)) {
            ...
            self.parentNode.querySelector('.submail').placeholder =
                'ENTER VALID EMAIL';
            return;
        }
    ...
    for (var i = 0; i < select.length; i++) {
        addEvent(select[i], 'click', function (e) {
            e = e || window.Event;
            e.stopPropagation();
